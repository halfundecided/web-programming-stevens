...
module.exports = {
    ...
    async addDog(name, breeds) {
        ...
        if(!breeds || !Array.isArray(breeds))
            throw "You musth provide an array of breeds";

        if (breeds.length === 0) throw "You must provide at least one bread.";
    ...
    async updateeDog(id, name, breeds) {
        if (!id) throw "You must provide an id to search for";
